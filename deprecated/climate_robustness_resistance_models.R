...
## *******************************************************************
# ---- TCM Robustness Figures (facet_grid: scenario × climate var) ----
## *******************************************************************
...
tcm_preds_api <- ggpredict(
  ...
  as.data.frame() %>%
  mutate(climate_var = "Antecedent Precipitation Index")
...
tcm_preds_anom <- ggpredict(
  ...
  as.data.frame() %>%
  mutate(climate_var = "Seasonal Temperature Anomaly")

tcm_preds_combined <- bind_rows(tcm_preds_api, tcm_preds_anom) %>%
  rename(scenario = group) %>%
  mutate(climate_var = factor(climate_var, levels = c(
...
p_tcm <- ggplot(
  ...
  facet_grid(scenario ~ climate_var, scales = "free_x") +
  coord_cartesian(
    ylim = range(c(tcm_preds_combined$conf.low,
                   tcm_preds_combined$conf.high))
  ) +
  theme_minimal(base_size = 13) +
  labs(
    x       = NULL,
    y       = "TCM Robustness",
    caption = "Marginal R² = 0.83 | Conditional R² = 0.89"
  ) +
  theme(
...
ggsave(
  "analysis/network/figures/TCM_Robustness_by_scenario.pdf",
  p_tcm,
  width = 10, height = 6
)

## *******************************************************************
# ---- SCM Robustness Figures (facet_grid: scenario × climate var) ----
## *******************************************************************
...
scm_preds_combined <- bind_rows(scm_preds_api, scm_preds_anom) %>%
  rename(scenario = group) %>%
  mutate(climate_var = factor(climate_var, levels = c(
    "Antecedent Precipitation Index",
    "Seasonal Temperature Anomaly"
...
ggsave(
  "analysis/network/figures/SCM_Robustness_by_scenario.pdf",
  p_scm,
  width = 10, height = 6
)
