...
predict.int <- function(mod,
                        ...
                        family="gaussian", 
                        orig.data = NULL){
  ## Ensure factor levels in prediction data match those in the original model data
  if (!is.null(orig.data)) {
    for (v in names(dd)) {
      if (v %in% names(orig.data) && is.factor(orig.data[[v]])) {
        dd[[v]] <- factor(as.character(dd[[v]]),
                          levels = levels(orig.data[[v]]))
      }
    }
  }
