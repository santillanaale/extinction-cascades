...
## ---- TMean Model (following Ramirez-Parada et al. 2023) ----
# Compute monthly mean temperature per site
monthly_temp <- all_temp_data %>%
  group_by(site_name, year, month) %>%
  summarize(
    TMEAN_month = mean((tmin + tmax)/2, na.rm = TRUE),
    .groups = "drop"
  )

### ---- TMean: 3-month antecedent TMEAN window ----
## Options: Use observed flowering dates in the dataset, use sampling month as a proxy, or define a median flowering month per species

TMean <- monthly_temp %>%
  filter(month %in% c(5,6,7)) %>%   # 3 months leading up to July
  group_by(site_name, year) %>%
  summarize(
    TMEAN = mean(TMEAN_month, na.rm = TRUE),
    .groups = "drop"
  )

### ---- TMEANNormal ----
# Load PRISM monthly Tmin/Tmax
long_term_temp <- read.csv(
  "PRISM_temp/PRISM_tmin_tmax_stable_800m_200801_202212.csv",
  skip = 10,
  stringsAsFactors = FALSE
)

# Rename Tmin and Tmax columns
long_term_temp <- long_term_temp %>% 
  rename(Tmin = tmin..degrees.C.) %>%
  rename(Tmax = tmax..degrees.C.) %>%
  rename(Site = Name)

# Parse as a first-of-month date (R needs a day)
long_term_temp$Date <- as.Date(paste0(long_term_temp$Date, "-01"))

# Extract year and month
long_term_temp$Year  <- year(long_term_temp$Date)
long_term_temp$Month <- month(long_term_temp$Date)

# Compute Monthly Mean
long_term_temp$Tmean <- (long_term_temp$Tmin + long_term_temp$Tmax) / 2

# Compute long-term monthly normals (15 years)
monthly_normals_by_site <- long_term_temp %>%
  group_by(Site, Month) %>%
  summarize(Tmean_normal = mean(Tmean, na.rm = TRUE))

# Compute long-term monthly normals (30 years)
# monthly_normals <- long_term_temp %>%
#   filter(Year >= 1991 & Year <= 2020) %>%
#   group_by(Site, Month) %>%
#   summarize(Tmean_normal = mean(Tmean, na.rm = TRUE))

# Merge normals back into long term dataset
long_term_temp <- long_term_temp %>%
  left_join(monthly_normals_by_site, by = c("Site", "Month"))

### ---- TMEANAnomaly ----
long_term_temp$Tmean_anomaly <- long_term_temp$Tmean - long_term_temp$Tmean_normal

# Plot of anomalies across all sites
ggplot(long_term_temp, aes(Date, Tmean_anomaly)) +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(y = "Tmean anomaly (°C)", x = "Date",
       title = "Monthly Temperature Anomalies (PRISM 2008–2022)")
ggsave("analysis/network/figures/Tmean_anomaly.pdf", width = 12, height = 4)

# Plot of anomalies by site
ggplot(long_term_temp, aes(Date, Tmean_anomaly)) +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  facet_wrap(~ Site, ncol = 4, scales = "free_y") +
  labs(y = "Tmean anomaly (°C)", x = "Date",
       title = "Site-specific Monthly Temperature Anomalies (PRISM 2008–2022)") +
  theme_bw()
ggsave("analysis/network/figures/Tmean_anomaly_site.pdf", width = 12, height = 4)


### ---- Aggregate Tmean and Anomaly for 3-month window ----
antecedent_Tmean <- long_term_temp %>%
  filter(Month %in% c(5,6,7)) %>%  # adjust for species flowering window
  group_by(Site, Year) %>%
  summarize(
    Tmean_window = mean(Tmean, na.rm = TRUE),
    Tmean_anomaly_window = mean(Tmean_anomaly, na.rm = TRUE),
    .groups = "drop"
  )

### ---- Modeling ----
# 1. Prepare antecedent temperature window for model
# Rename TMean dataframe columns to match the model
antecedent_Tmean <- TMean %>%
  rename(Site = site_name) %>%
  rename(TMEANNormal = TMEAN)  # long-term 3-month mean placeholder

# If you want TMEANAnomaly for the same 3-month window, calculate like this:
antecedent_anomaly <- TMean %>%
  rename(TMEANNormal = TMEAN, Site = site_name) %>%
  left_join(long_term_temp %>% 
              group_by(Site, Year, Month) %>%
              summarize(Tmean_normal_window = mean(Tmean_normal[Month %in% c(5,6,7)], na.rm = TRUE),
                        .groups = "drop"),
            by = c("Site" = "Site", "year" = "Year")) %>%
  mutate(TMEANAnomaly = TMEANNormal - Tmean_normal_window) %>%
  select(Site, year, TMEANNormal, TMEANAnomaly)



model_df <- flower_data %>%
  left_join(antecedent_Tmean, by = c("Site", "Year")) %>%
  rename(
    TMEANNormal = Tmean_window,        # long-term mean
    TMEANAnomaly = Tmean_anomaly_window
  ) %>%
  drop_na(DOY, TMEANNormal, TMEANAnomaly)
